...
export default function Home() {
  ...
  async function fetchPlaces() {
    setLoading(true);
    const { data, error } = await supabase.from("places").select("*").order("match_score", { ascending: false }).limit(10);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-900 text-white">
      <h1 className="text-3xl font-bold mb-6">🔥 Find Your Next Spot</h1>
